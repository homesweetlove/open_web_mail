...
단, 이미 해당 포트를 다른 프로그램이 사용하고 있다면 Vite가 다른 포트를 선택할 수 있으므로 터미널 출력 주소를 확인하는 것이 가장 정확합니다.
...
프로덕션에 가까운 형태로 Vite 빌드 결과를 간단하게 확인하고 싶다면 다음 명령을 사용할 수 있습니다.
...
현재 메일 데이터는 실제 서버에서 가져오는 데이터가 아닙니다.

UI 테스트를 위해 코드 내부에 포함되어 있으므로, 내용이 변경되거나 초기화되어도 실제 메일에는 아무 영향이 없습니다.
...
현재 UI 상태 대부분은 클라이언트 메모리 기준입니다.

따라서 페이지를 새로고침하면 일부 조작 상태가 초기화될 수 있습니다.
